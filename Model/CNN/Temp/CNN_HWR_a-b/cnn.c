...
void DBfeedforward(float *in_mat, uint8_t *predicted_num, float *predicted_num_confidence)
{
	/*
	Matrix/Array to store calculated value:
		One array of 9140 float value = 36560 bytes = about 37Kbytes
	...
	*/
	...
	FILE *file = fopen("log002_B.txt", "w"); // Open file for writing
	if (file == NULL) 
	{
	...
	fprintf(file, "After Conv1:\n");
    printMatrixToFile(file, wtf_mat, CONV1_K_NUM, AFTER_CONV1_DIM, printKernelIdx);
    fprintf(file, "\n\n");

	relu(wtf_mat, AFTER_CONV1_DIM, CONV1_K_NUM);

	fprintf(file, "After ReLU1:\n");
    printMatrixToFile(file, wtf_mat, CONV1_K_NUM, AFTER_CONV1_DIM, printKernelIdx);
    fprintf(file, "\n\n");
	...
	fprintf(file, "After Conv2:\n");
    printMatrixToFile(file, &wtf_mat[ARRAY_SEPARATOR01], CONV2_K_NUM, AFTER_CONV2_DIM, printKernelIdx);
    fprintf(file, "\n\n");

	relu(&wtf_mat[ARRAY_SEPARATOR01], AFTER_CONV2_DIM, CONV2_K_NUM);

	fprintf(file, "After layer2-ReLU:\n");
    printMatrixToFile(file, &wtf_mat[ARRAY_SEPARATOR01], CONV2_K_NUM, AFTER_CONV2_DIM, printKernelIdx);
    fprintf(file, "\n\n");
	
	maxpooling2x2(&wtf_mat[ARRAY_SEPARATOR01], wtf_mat, AFTER_CONV2_DIM, CONV2_K_NUM);

	fprintf(file, "After layer2-maxpool:\n");
    printMatrixToFile(file, wtf_mat, CONV2_K_NUM, AFTER_MAXP1_DIM, printKernelIdx);
    fprintf(file, "\n\n");

	// layer 3 ================================================
	conv2d(wtf_mat, &wtf_mat[ARRAY_SEPARATOR02], (float *)conv3_filter_weight, (float *)conv3_filter_bias, AFTER_MAXP1_DIM, CONV2_K_NUM, CONV3_K_NUM);
	relu(&wtf_mat[ARRAY_SEPARATOR02], AFTER_CONV3_DIM, CONV3_K_NUM);	

	fprintf(file, "After layer3-ReLU:\n");
    printMatrixToFile(file, &wtf_mat[ARRAY_SEPARATOR02], CONV3_K_NUM, AFTER_CONV3_DIM, printKernelIdx);
    fprintf(file, "\n\n");

	// layer 4 ================================================
	conv2d(&wtf_mat[ARRAY_SEPARATOR02], &wtf_mat[ARRAY_SEPARATOR03], (float *)conv4_filter_weight, (float *)conv4_filter_bias, AFTER_CONV3_DIM, CONV3_K_NUM, CONV4_K_NUM);
	relu(&wtf_mat[ARRAY_SEPARATOR03], AFTER_CONV4_DIM, CONV4_K_NUM);

	fprintf(file, "After layer4-ReLU:\n");
    printMatrixToFile(file, &wtf_mat[ARRAY_SEPARATOR03], CONV4_K_NUM, AFTER_CONV4_DIM, printKernelIdx);
    fprintf(file, "\n\n");	// OK till here

	maxpooling2x2(&wtf_mat[ARRAY_SEPARATOR03], &wtf_mat[ARRAY_SEPARATOR04], AFTER_CONV4_DIM, CONV4_K_NUM);
	// wtf_mat[ARRAY_SEPARATOR04] is already pentan -> no need flatten

	fprintf(file, "After layer4-maxpool:\n");
    printMat1DToFile(file, &wtf_mat[ARRAY_SEPARATOR04], FC01NODENUM);
    fprintf(file, "\n\n");

	// Fullyconnected 1 =======================================
	fullyconnected(&wtf_mat[ARRAY_SEPARATOR04], &wtf_mat[ARRAY_SEPARATOR05], (float *)lin1_weight, (float *)lin1_bias, FC01NODENUM, FC02NODENUM);	

	fprintf(file, "After Fullyconnected 1:\n");
    printMat1DToFile(file, &wtf_mat[ARRAY_SEPARATOR05], FC02NODENUM);
    fprintf(file, "\n\n");

	// Fullyconnected 2 =======================================
	fullyconnected(&wtf_mat[ARRAY_SEPARATOR05], &wtf_mat[ARRAY_SEPARATOR06], (float *)lin2_weight, (float *)lin2_bias, FC02NODENUM, OUTPUT_SIZE);	

	fprintf(file, "After Fullyconnected 2:\n");
    printMat1DToFile(file, &wtf_mat[ARRAY_SEPARATOR06], OUTPUT_SIZE);
    fprintf(file, "\n\n");

	// Soft_expensive ================================================
	softmax(&wtf_mat[ARRAY_SEPARATOR06], &wtf_mat[ARRAY_SEPARATOR07], OUTPUT_SIZE);

	fprintf(file, "After Softmax:\n");
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        fprintf(file, "%.6f ", wtf_mat[ARRAY_SEPARATOR07 + i]);
    }
    fprintf(file, "\n\n");
	...
	fprintf(file, "Predicted Number: %u\n", *predicted_num);
    fprintf(file, "Predicted Confidence: %.6f\n", *predicted_num_confidence);
...
/*************************** Debug functions ***************************/
void printMatrixToFile(FILE *file, float *mat, int num_ch, int mat_size, int external_arg)
{
    for (int i_ch = 0; i_ch < num_ch; i_ch++)
    {
        // External argument for printing extra info - mainly for debugging purposes
        if (external_arg == printKernelIdx)
        {
        ...
        for (int i = 0; i < mat_size; i++)
        {
            for (int j = 0; j < mat_size; j++)
            {
                fprintf(file, "%.3f  ", *(mat + (i_ch * mat_size * mat_size) + (i * mat_size) + j));
            }
            fprintf(file, "\n");
        }
        fprintf(file, "\n");
...
void printFilterBiasToFile(FILE *file, float *bias_mat, int num_ch)
{
    for (int i_ch = 0; i_ch < num_ch; i_ch++)
    {
        fprintf(file, "%f ", bias_mat[i_ch]);
    }
    fprintf(file, "\n\n");
}

void printMat1DToFile(FILE *file, float *mat, int num_ch)
{
    printFilterBiasToFile(file, mat, num_ch);
}

void printFilterWeightToFile(FILE *file, float *mat, int num_ch_out, int num_ch_in)
{
    int mat_size = FILTER_SIZE;
    for (int i_ch_out = 0; i_ch_out < num_ch_out; i_ch_out++)
    {
        fprintf(file, "Kernel %d\n", i_ch_out);
        printMatrixToFile(file, mat + i_ch_out * num_ch_in * mat_size * mat_size, num_ch_in, mat_size, printChannelIdx);
    }
}
